...
test('drive.version', async (t) => {
  const { drive } = await testenv(t)
  await drive.put(sampleFile, fs.readFileSync(sampleFile))
...
test('drive.createWriteStream(path) and drive.createReadStream(path)', async (t) => {
  {
    const { drive } = await testenv(t)
    const diskbuf = await fs.readFileSync(sampleFile)
    await pipeline(fs.createReadStream(sampleFile), drive.createWriteStream(sampleFile))
    let bndlbuf = null
    await pipeline(
      drive.createReadStream(sampleFile),
      new Writable({
        write(data, cb) {
...
test('drive.del() deletes entry at path', async (t) => {
  ...
  const { drive } = await testenv(t)
  await drive.put(sampleFile, fs.readFileSync(sampleFile))
  let buf = await drive.get(sampleFile)
  t.ok(b4a.isBuffer(buf))
  await drive.del(sampleFile)
  buf = await drive.get(sampleFile)
  t.is(buf, null)
  const entry = await drive.entry(sampleFile)
  t.is(entry, null)
})
...
test('drive.symlink(from, to) updates the entry at <from> to include a reference for <to>', async (t) => {
  ...
  await drive.put(sampleFile, buf)
  await drive.symlink('pointer', sampleFile)
  const result = await drive.get('pointer')
  t.is(result, null)
  const entry = await drive.entry('pointer')
  t.is(entry.value.linkname, sampleFile)
...
test('drive.entry(path) gets entry at path', async (t) => {
...

  {
    ...
    const buf = fs.readFileSync(sampleFile)
    await drive.put(sampleFile, buf)
    const { value: entry } = await drive.entry(sampleFile)
    t.ok(entry.blob)
...

  {
    const { drive } = await testenv(t)
    const buf = fs.readFileSync(sampleFile)
    await drive.put(sampleFile, buf, { executable: false })
    const { value: entry } = await drive.entry(sampleFile)
    t.ok(entry.blob)
    t.is(entry.linkname, null)
    t.is(entry.executable, false)
...

  {
    const { drive } = await testenv(t)
    const buf = fs.readFileSync(sampleFile)
    await drive.put(sampleFile, buf, { executable: true })
    const { value: entry } = await drive.entry(sampleFile)
    t.ok(entry.blob)
    t.is(entry.linkname, null)
...

  {
    ...
    const buf = fs.readFileSync(sampleFile)
    await drive.put(sampleFile, buf, { executable: false })
    await drive.symlink(sampleFile, linkname)
    const { value: entry } = await drive.entry(sampleFile)
  ...
  {
    const { drive } = await testenv(t)
    await drive.symlink(linkname, sampleFile)
    const { value: entry } = await drive.entry(linkname)
    t.is(entry.blob, null)
    t.is(entry.executable, false)
    t.is(entry.linkname, sampleFile)
  }

  {
    const { drive } = await testenv(t)
    const ws = drive.createWriteStream(sampleFile)
    ws.write(fs.readFileSync(sampleFile))
    ws.end()
  ...
  {
    const { drive } = await testenv(t)
    const ws = drive.createWriteStream(sampleFile, { executable: false })
    ws.write(fs.readFileSync(sampleFile))
    ws.end()
    await once(ws, 'finish')
    const { value: entry } = await drive.entry(sampleFile)
  ...
  {
    const { drive } = await testenv(t)
    const ws = drive.createWriteStream(sampleFile, { executable: true })
    ws.write(fs.readFileSync(sampleFile))
    ws.end()
    await once(ws, 'finish')
    const { value: entry } = await drive.entry(sampleFile)
