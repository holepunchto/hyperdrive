...
A design/specification document for Hyperdrive.

Hyperdrive is a protocol / network for distributing and replicating
feeds of binary using merkle trees to deduplicate and guarantee content integrety

A classic use-case of this is replicating file and directories which is also discussed in
this document.

The protocol itself draws heavy inspiration from existing file sharing systems such as BitTorrent
and [ppspp](https://tools.ietf.org/html/rfc7574)
...
In general payload messages are small and an implementation should make sure to only buffer messages that can easily fit in memory. The javascript module currently rejects all messages >5mb.
...
`channel` should be set to the lowest locally available channel number (starting at `0`) and all subsequent messages sent refering to the same `link` should contain the same channel number. When receiving a Join message, if you wish to join the swarm, you should reply back with a new Join message if you haven't sent one already containing the same link a your lowest locally available channel number.
