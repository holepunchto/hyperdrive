...
function readdirIterator (drive, db, name, opts) {
  const recursive = !!(opts && opts.recursive)
  const noMounts = !!(opts && opts.noMounts)
  const includeStats = !!(opts && opts.includeStats)

  const ite = statIterator(drive, db, name, { ...opts, recursive, noMounts, gt: false })
...
function createReaddirStream (drive, db, path, opts) {
  return toStream(readdirIterator(drive, db, path, opts))
}

function createStatStream (drive, db, path, opts) {
  return toStream(statIterator(drive, db, path, opts))
}

function createMountStream (drive, db, opts) {
  return toStream(mountIterator(drive, db, opts))
}
