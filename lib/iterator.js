...
function mountIterator (drive, opts) {
  ...
  return nanoiterator({
    ...
    next (cb) {
      if (first) {
        first = null
        console.error('drive metadata length:', drive.metadata.length)
        if (drive.metadata.has(0)) {
          console.error('getting first content')
          return drive._getContent(drive._db, err => {
            console.error('got first content')
            if (err) return cb(err)
            return cb(null, {
              path: '/',
      ...
      ite.next((err, val) => {
        ...
        console.error('getting mountpoint content')
        return drive._getContent(val.trie, (err, contentState) => {
          console.error('got mountpoint content')
          if (err) return cb(err)
