...
class FileDescriptor {
  ...
  write (buffer, offset, len, pos, cb) {
    ...
    function doWrite (err) {
      ...
      console.log('WRITING', len, 'at pos:', pos, 'offset:', offset, 'slice:', slice, 'feed length:', self.drive.contentFeed.length)
  ...
  _seekAndRead (buffer, offset, len, pos, cb) {
    ...
    this.drive.contentFeed.seek(this.stat.byteOffset + pos, { start, end }, (err, blk, blockOffset) => {
      ...
      console.log('this.stat:', this.stat, 'seeked to:', pos, 'start:', start, 'end:', end)
      this._read(buffer, offset, len, cb)
    })
  }

  _read (buffer, offset, len, cb) {
    console.log('at beginning of read, stat:', this.stat)
    const self = this
    ...
    function readNextBlock () {
      self._readBlock(buffer, offset + totalRead, Math.max(available, 0), (err, bytesRead) => {
        ...
        console.log('AT END OF READ, buffer:', buffer)
  ...
  _readBlock (buffer, offset, len, cb) {
    ...
    console.log('reading block at position:', blk)
    ...
    this.drive.contentFeed.get(blk, (err, data) => {
      ...
      console.log('blkOffset:', blkOffset, 'blk:', blk, 'data:', data)
...
FileDescriptor.create = function (drive, name, flags, cb) {
  ...
  console.log('readable:', readable, 'writable:', writable, 'appending:', appending, 'creating:', creating)
