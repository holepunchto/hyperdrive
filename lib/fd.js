...
class FileDescriptor {
  constructor (drive, path, stat, readable, writable, appending, creating) {
    ...
    this._batcher = byteStream({ time: 500, limit: 4096 * 100 })
  ...
  write (buffer, offset, len, pos, cb) {
    ...
    if (!this._writeStream) {
      const writeStream = this.drive.createWriteStream(this.path)
      this._writeStream = pumpify(this._batcher, through.obj((chunk, enc, cb) => {
    ...
    function doWrite (err) {
      ...
      console.log('writing slice:', slice, 'to batcher')
  ...
  _refreshDownload (start, cb) {
    let end = Math.min(this.stat.blocks + this.stat.offset, start + 500)
    ...
    console.log(`DOWNLOADING FROM ${start} TO ${end} randomly`)
    this._range = this.drive.contentFeed.download({ start, end, linear: true }, cb || noop)
  }
  ...
  _read (buffer, offset, len, cb) {
    ...
    function readNextBlock () {
      self._readBlock(buffer, offset + totalRead, Math.max(available, 0), (err, bytesRead) => {
        ...
        console.log('totalRead:', totalRead, 'len:', len)
  ...
  _readBlock (buffer, offset, len, cb) {
    ...
    console.log('GETTING BLOCK:', blk)
    this.drive.contentFeed.get(blk, (err, data) => {
      console.log('GOT BLOCK:', blk)
      if (err) return cb(err)
