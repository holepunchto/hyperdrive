  ...
  write (buffer, offset, len, pos, cb) {
    ...
    if (!this._writeStream) {
      ...
      if (appendStream) {
        ...
        appendStream.pipe(this._writeStream, { end: false })

        appendStream.on('error', err => this._writeStream.destroy(err))
        this._writeStream.on('error', err => appendStream.destroy(err))

        return appendStream.on('end', doWrite)
      }
    }
    doWrite()
...
FileDescriptor.create = function (drive, name, flags, cb) {
  ...
  drive.contentReady(err => {
    ...
    drive._db.get(name, (err, st) => {
      ...
      if (st) st = messages.Stat.decode(st.value)
