...
class FileDescriptor {
  ...
  _refreshDownload (start, cb) {
    ...
    this._range = this.drive.content.download({ start, end, linear: true }, cb || noop)
  }
  ...
  _seekAndRead (buffer, offset, len, pos, cb) {
    ...
    this.drive.content.seek(this.stat.byteOffset + pos, { start, end }, (err, blk, blockOffset) => {
      if (err) return cb(err)
      this.position = pos
  ...
  _readBlock (buffer, offset, len, cb) {
    ...
    this.drive.content.get(blk, (err, data) => {
      if (err) return cb(err)
...
function createWriteStream (drive, path) {
