...
class FileDescriptor {
  ...
  close (cb) {
    ...
    if (this._range) {
      return this.contentState.feed.undownload(this._range, err => {
        if (err) return cb(err)
        // TODO: Ensure that the range being cancelled isn't being read by another operation.
        return cb(null)
        /*
        this.contentState.feed.cancel(this._range.start, this._range.end, err => {
          if (err) return cb(err)
          return cb(null)
        })
        */
  ...
  _refreshDownload (start, cb) {
    ...
    const refresh = () => this.contentState.feed.download({ start, end, linear: true }, cb || noop)
