...
class FileDescriptor {
  ...
  read (buffer, offset, len, pos, cb) {
    console.log('READING', len, 'at', pos, 'with stat:', this.stat)
  ...
  write (buffer, offset, len, pos, cb) {
    ...
    if (this.appending && pos < this.stat.size) {
      console.log('pos:', pos, 'this.stat.size:', this.stat.size)
  ...
  _read (buffer, offset, len, cb) {
    ...
    this.drive.contentFeed.get(blk, (err, data) => {
      ...
      console.log('copying data:', data, 'into buf')
      data.copy(buf)
...
FileDescriptor.create = function (drive, name, flags, cb) {
  ...
  console.log(`FD FOR ${name} append? ${appending} readable? ${readable} writeable? ${writable}`)
