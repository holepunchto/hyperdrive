...
class FileDescriptor {
  ...
  _read (buffer, offset, len, cb) {
    ...
    function readNextBlock () {
      self._readBlock(buffer, offset + totalRead, available, (err, bytesRead) => {
        if (err) return cb(err)
...
FileDescriptor.create = function (drive, name, flags, cb) {
  ...
  drive.contentReady(err => {
    ...
    drive._db.get(name, (err, st) => {
      ...
      return cb(null, new FileDescriptor(drive, name, st, readable, writable, appending, creating))
    })
  })
}
