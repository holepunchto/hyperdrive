...
module.exports = class Download extends ReadyResource {
    ...
    if (this.folder !== null) {
      ...
      for await (const entry of drive.list(this.folder, this.options)) {
        ...
        if (blob.blockMap && blob.blockLength) {
          this.maps.push(blobs.core.download({ start: blob.blockOffset, length: blob.blockLength }))
        }
        entries.push(entry) // cache entries
      }

      const results = await Promise.all(this.maps.map((m) => this._cancellableDownload(m)))
  ...
  async _downloadDiff(blobs) {
    ...
    this.stream = this.drive.diff(this.diff, this.folder, this.options)
    ...
    for await (const entry of this.stream) {
      ...
      if (blob.blockMap && blob.blockLength) {
        this.maps.push(this.constructor.downloadEntryMap(blobs, blob))
      }
