...
function FeedState (drive, id, opts) {
  if (!(this instanceof FeedState)) return new FeedState(drive, id, opts)
  if (!opts) opts = {}
...
util.inherits(FeedState, events.EventEmitter)

FeedState.prototype.proof = function (block, have, cb) {
...
FeedState.prototype.putRoots = function (block, data, proof, cb) {
  if (!cb) cb = noop
...
FeedState.prototype.put = function (block, data, proof, cb) {
  if (!cb) cb = noop
...
FeedState.prototype._verifyRoots = function (proof) {
...
FeedState.prototype.get = function (block, cb) {
