...
Channel.prototype.parse = function (data) {
  ...
  switch (type) {
    case 0: return this._onbitfield(message)
    case 1: return this._onhave(message)
    case 2: return this._onrequest(message)
    case 3: return this._onresponse(message)
  }

  this.emit('warn', new Error('Unknown message type: ' + type))
}

Channel.prototype.request = function (block, tree, cb) {
  var req = {
    id: 0,
    block: block,
    tree: tree,
...
Channel.prototype.response = function (req, block, tree) {
  this._push(3, {
    id: req.id,
    block: block,
    tree: tree
...
Channel.prototype.bitfield = function (blocks) {
  this._push(0, {
    blocks: blocks.buffer || blocks
  })
}
...
Channel.prototype._onbitfield = function (message) {
  this.remoteBlocks = bitfield(message.blocks, {grow: MAX_BLOCKS})
  this.emit('bitfield', this.remoteBlocks)
}
