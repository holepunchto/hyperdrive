...
Channel.prototype.onresponse = function (message) {
  ...
  if (this.amRequesting.get(block)) this.inflight--
...
a.on('channel', function (channel) {
  console.log('a have a channel', channel.link)
  ...
  channel.on('request', function (block) {
    console.log('b requests', block)

    // var want = 2 * block
    // var sibling = flat.sibling(want)
    // var hashes = []
    // var bits = channel.remoteBitfield

    // while (!bits.get(want) && flat.rightSpan(want) <= head) {
    //   if (!bits.get(sibling)) hashes.push(sibling)
    //   want = flat.parent(sibling)
    //   sibling = flat.sibling(want)
    // }

    // if (!bits.get(want)) {
    //   var roots = flat.fullRoots(head + 2)
    //   for (var i = 0; i < roots.length; i++) hashes.push(roots[i])
    // }

    // console.log(hashes)
  })

  channel.on('unchoke', function () {
    console.log('b unchoked a')
    channel.unchoke()
  })
...
b.on('channel', function (channel) {
  console.log('b have a channel', channel.link)
  channel.on('leave', function () {
    console.log('b left')
  })

  channel.on('unchoke', function () {
    console.log('a unchoked b')
    channel.request(0)
  })

  channel.on('have', function () {
    console.log('a have some blocks', channel.remoteBitfield)
  })
...
a.join(new Buffer('link'))
a.join(new Buffer('link'))

a.pipe(b).pipe(a)

// var id = p.interested(new Buffer('link'))
...
// p.on('interested', function (id, link) {
//   console.log(p.get(id))
// })

// p.interested(<local-id>)
// p.uninterested(<local-id>)

// p.choke(<remote-id>)
// p.unchoke(<remote-id>)
// p.response(<remote-id>)
// p.info(<remote-id>)
// p.have(<remote-id>)
// p.request(<local-id>)

// p.remoteHave(<remote-id>)
// p.remoteInterested(<remote-id>)
// p.remoteChoking(<local-id>)

// p.localInterested(<local-id>)
// p.localChoking(<remote-id>)

// p.local(<local-id>) --> remoteChoking: true, remoteBitfield: <...>, link: ...
// p.remote(<remote-id>) --> choking: true, link: ...
