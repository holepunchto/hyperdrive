...
function writeStream (hyperdrive, opts) {
  ...
  var indexer = opts.index ? indexStream() : null

  var ws = pumpify.obj(indexer ? [indexer, merkle, batcher] : [merkle, batcher])
...
var ws = writeStream(db, {index: true})
...
ws.end(function () {
  console.log(ws.id, ws.blocks, ws.index)
})
